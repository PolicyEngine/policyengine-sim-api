"""Tests for direct, durable Stage 12 evaluation dispatch."""
...
from policyengine_simulation_entry.stage12_backend import Stage12EvaluationBackend
...
class FakeControl:
    ...
    def enabled(self, *, calculation_flow, environment):
        assert calculation_flow == "economy"
...
def test_parent_is_durable_before_direct_modal_dispatch() -> None:
    ...
    _backend(store, invoker)._dispatch(
    ...
    assert store.created[0].status is EvaluationLifecycleStatus.PENDING
    assert store.replaced[0].status is EvaluationLifecycleStatus.RUNNING
    assert store.replaced[0].coordinator_invocation_id.startswith("dispatch-pending-")
    assert store.current.status is EvaluationLifecycleStatus.RUNNING
...
def test_failed_dispatch_reuses_parent_version_and_deterministic_children() -> None:
    ...
    invoker = FakeInvoker()
    loader = FakeLoader()
    backend = Stage12EvaluationBackend(
...
def test_concurrent_create_resolution_does_not_dispatch_a_second_coordinator() -> None:
    ...
    _backend(first_store, first_invoker)._dispatch(
        request_payload=eligible_payload(),
        production_response=_response(),
        request_id="request-1",
    )
...
def test_disabled_runtime_control_performs_no_evaluation_work() -> None:
    store = FakeStore()
    invoker = FakeInvoker()
    backend = Stage12EvaluationBackend(
        make_settings(environment="staging"),
        manifest_loader=FakeLoader(),
        control_loader=FakeControl(enabled=False),
        store=store,
        invoker=invoker,
    )
...
def test_unsupported_input_records_one_skip_without_dispatch() -> None:
    ...
    assert store.created[0].status is EvaluationLifecycleStatus.SKIPPED
...
def test_dispatch_failure_is_sanitized_and_persisted() -> None:
    ...
    with pytest.raises(
        RuntimeError, match="Stage 12 evaluation dispatch failed"
    ) as error:
        _backend(store, invoker)._dispatch(
            request_payload=eligible_payload(),
            production_response=_response(),
    ...
    assert failed.status is EvaluationLifecycleStatus.FAILED
    assert failed.error_code == "evaluation_dispatch_failed"
...
def test_temporary_submission_spawns_without_waiting_for_modal_result() -> None:
    ...
    report = backend._submit_temporary_report(
        request_payload=eligible_payload(),
    ...
    assert report.status is EvaluationLifecycleStatus.RUNNING
...
def test_each_temporary_submission_creates_a_distinct_report() -> None:
    ...
    first = _backend(first_store, FakeInvoker())._submit_temporary_report(
        request_payload=eligible_payload(),
        request_id="request-1",
    )
    second = _backend(second_store, FakeInvoker())._submit_temporary_report(
        request_payload=eligible_payload(),
        request_id="request-2",
    )
...
def test_temporary_submission_rejects_unsupported_input_without_persistence() -> None:
    ...
    with pytest.raises(TemporaryStage12UnsupportedRequest) as error:
        _backend(store, FakeInvoker())._submit_temporary_report(
...
def test_temporary_status_reads_postgres_state_without_modal_access() -> None:
    store = FakeStore()
    invoker = FakeInvoker()
    report = _backend(store, invoker)._submit_temporary_report(
        request_payload=eligible_payload(),
        request_id="request-1",
    )
    store.events.clear()
...
def test_temporary_status_rejects_a_record_from_another_environment() -> None:
    store = FakeStore()
    report = _backend(store, FakeInvoker())._submit_temporary_report(
        request_payload=eligible_payload(),
        request_id="request-1",
    )
    production_backend = Stage12EvaluationBackend(
...
async def test_temporary_submission_offloads_synchronous_dispatch(monkeypatch) -> None:
    ...
    calls = []
    original = backend._submit_temporary_report
